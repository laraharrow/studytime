    ...
    <div>
      <Paper className={classes.root} elevation={1}>
        <Button
          ...
          <br/>
          <Typography className={classes.align} variant="body1" gutterBottom>
            This structure like linked list is a sequence of nodes linked one to another. But the similarites end right there. Trees have a hierarchical structure where each node has its own data value, a parent node, and a childrens list usually represented by an array of nodes. One more important detail to be aware is that each node can not have more then one parent but it can have muitiple children.
            <br/>
            ...
            <br/>
            There is a very useful Tree, that we will be implementing here, called <strong className={classes.code}>Binary Search Tree - BST</strong>. Everything that we talked about so far relating to trees also apply to a <strong className={classes.code}>BST</strong> tree, exept that every tree parent can only have two children. Both chidren will be placed based on its relationship to the parent tree data. If the new tree introduces a data value greter then its parent tree data. Then that new tree will be placed on the right side of the tree. Now if the new tree has a data value lower then the parent tree data that new tree will be places on the left side.
            <br/>
            ...
            <br/>
            There are two main ways to traverse through every node in a <strong className={classes.code}>BST</strong>, that is, look into every node in the tree. There are some subdivision that we can get into when we write the code. But for now we just want to get some understanding of how the two traversing methods work.
            <br/>
            ...
            <br/>
            Each of those traverse methods refer to the one of the first structures we looked at. <strong className={classes.code}>BFT</strong> uses a stack and <strong className={classes.code}>DFT</strong> uses a queue to check each node in the tree. Just keep that in mind when we are looking into the code.
            <br/>
            <br/>
